...
int main(){
    ...
    int isValid = 0; /*set to zero and if stays as zero then valid else not valid*/
...
void *isValid(void* checkRC){
    ...
    int i, j;
    int rowSum; /*will hold value of the row and the column*/
    int colSum;
    //check
   ...
   // printf("checking row: %d || col: %d\n", row, col);
    ...
    if(rowSum != CORRECT_SUM){          /* if not equal then set row bit to 1.
